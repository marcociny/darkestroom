...
void Player::addLighterFuel(int val) {
    lighterFuel += val;
    lighterFuel = min(lighterFuel, maxLighterFuel);
    return;
}

void Player::setLighterFuel(int val) {
    lighterFuel = val;
    lighterFuel = min(lighterFuel, maxLighterFuel);
    return;
}
...
void Game::loadFloor(int floor_n) {

    Player plr = gameMap.getPlayer();
    floor_data floor = gameMap.getFloor();
    ...
    gameMap.changeRoom(floor_starting_rooms[floor_n], floor_starting_x[floor_n], floor_starting_y[floor_n]);
    
    coord playerPos = plr.getPos();
    playerPos.x = floor_starting_x[floor_n];
    playerPos.y = floor_starting_y[floor_n];

    gameMap.changeFloor(floor_n, msgs[LANG_OPTION]["Floor0Splash"]); // bad, fix
    return;
}  
...
int Game::game () {
    ...
    coord playerPos = gameMap.getPlayer().getPos();

    gameMap.changeFloor(currentFloor, msgs[LANG_OPTION][floor_splash]);
    gameMap.changeRoom(currentRoom, playerPos.x, playerPos.y);

    gameMap.initFloorPickups(currentFloor);
    ...
    while(1) {
        ...
        if(DEBUG_MODE == 3) {
            wattron(win, COLOR_PAIR(3));
            wprintw(win, "\n\n%s\n", gameMap.getFloor().room[currentRoom].art[0].c_str());
            wattroff(win, COLOR_PAIR(3));
        }
        if(DEBUG_MODE == 4) {
            mvwprintw(message_log, 2, COLS * 0.91, "%f", plr.lighterFuel);
            mvwprintw(message_log, 2, COLS * 0.20, "%d", in);
            mvwprintw(message_log, 2, COLS * 0.25, "%d", currentRoom);
        }
        ...
        wrefresh(message_log);
        wclear(message_log);
        gui.getMessageLog().printMessages(message_log);

        // lighter fuel and health GUI
        {
            mvwprintw(message_log, 2, COLS * 0.88, msgs[LANG_OPTION]["Fuel"].c_str());
            int consumption_speed = FRAMES_PER_SECOND/(plr.lighterFuelConsumption*4);
            int regen_speed = (FRAMES_PER_SECOND/(plr.lighterFuelRegen*4));
            ...
            if(frame % (regen_speed == 0 ? 1 : consumption_speed) == 0) {
                if(!plr.isLighterOn() && plr.lighterFuel < plr.maxLighterFuel){
            ...
            for(int i = 0; i < plr.maxLighterFuel; i+=5) {
                // █ ▓ ░ ▮▯
                if(plr.lighterFuel - i <= 0) {
                    mvwprintw(message_log, 3, COLS * 0.88 + i/5, "░");
                }
                else if(plr.lighterFuel - i < 5) {
                    mvwprintw(message_log, 3, COLS * 0.88 + i/5, "█");
                }
                else if(plr.lighterFuel - i >= 5) {
                    mvwprintw(message_log, 3, COLS * 0.88 + i/5, "█");
            ...
            for(int i = 0; i < plr.maxHealth; i+=5) {
                if(plr.health - i <= 0) {
                    mvwprintw(message_log, 7, COLS * 0.88 + i/5, "░");
                }
                else if(plr.health - i < 5) {
                    mvwprintw(message_log, 7, COLS * 0.88 + i/5, "█");
                }
                else if(plr.health - i >= 5) {
                    mvwprintw(message_log, 7, COLS * 0.88 + i/5, "█");
...
auto& active_room = gameMap.getFloor().room[currentRoom];
...
void Game::move_player(int& in) {
    ...
    if(plr.isMovementLocked()) return;
    ...
    coord pos = plr.getPos();
    eventHandler(pos.x + x, pos.y + y);
    return;
}
...
void Game::renderMap(WINDOW *win) {
    ...
    if(plr.isLighterOn()) {
        (playerPos.x, playerPos.y, plr.lighterStrength);
        plr.lighterFuel -= plr.lighterFuelConsumption/FRAMES_PER_SECOND;
        if(plr.lighterFuel <= 0) {
            if(plr.isLighterOn()) {
                plr.toggleLighter();
            }
        }
        ...
        plr.addLighterFuel(plr.lighterFuelRegen/FRAMES_PER_SECOND);
    }
